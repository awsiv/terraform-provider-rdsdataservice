...
import (
	...
	dataapi "github.com/aws/aws-sdk-go/service/rdsdataservice"
...
func resourceAwsRdsdataservicePostgresDatabaseCreate(d *schema.ResourceData, meta interface{}) error {
	dataapiconn := meta.(*AWSClient).dataapiconn

	sql := fmt.Sprintf("CREATE DATABASE %s OWNER %s;",
		d.Get("name").(string),
		d.Get("owner").(string))

	createOpts := dataapi.ExecuteStatementInput{
	...
	_, err := dataapiconn.ExecuteStatement(&createOpts)
...
func resourceAwsRdsdataservicePostgresDatabaseExists(d *schema.ResourceData, meta interface{}) (bool, error) {
	...
	output, err := dataapiconn.ExecuteStatement(&createOpts)
...
func rdsDataserviceExecuteStatement(d *schema.ResourceData, sql string, meta interface{}) (*dataapi.ExecuteStatementOutput, error) {
	dataapiconn := meta.(*AWSClient).dataapiconn
